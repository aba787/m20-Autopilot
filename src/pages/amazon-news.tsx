    ...
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-white flex items-center gap-2">
          <Newspaper className="w-5 h-5" style={{ color: 'var(--accent)' }} /> Amazon News
        </h1>
        <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Latest updates and important news for sellers</p>
      </div>
              ...
              <div key={n.id} className="p-4 hover:shadow-md transition-all rounded-xl"
                ...
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  <span className="ml-auto text-xs px-1.5 py-0.5 rounded font-medium"
                    style={{ background: 'rgba(245,158,11,0.12)', color: 'var(--warning)', border: '1px solid rgba(245,158,11,0.25)' }}>⭐ Important</span>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="text-xs px-2 py-0.5 rounded font-medium"
                      style={categoryColors[n.category] ?? defaultCategory}>{n.category}</span>
                    <span className="text-xs" style={{ color: 'var(--text-dim)' }}>{n.date}</span>
                    {n.important && <span className="text-xs" style={{ color: 'var(--warning)' }}>⭐</span>}
                  </div>
