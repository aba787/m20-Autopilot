...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    <div style="font-family:sans-serif;max-width:700px;margin:0 auto;background:#0f0a1a;color:#e2e8f0;padding:32px;border-radius:12px;">
      ...
      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
        <tr><td style="padding:8px 12px;background:#1e1030;color:#94a3b8;width:180px;border-radius:6px 0 0 6px;">Severity</td>
            <td style="padding:8px 12px;background:#1a0f2e;font-weight:700;color:${severity === 'critical' ? '#ef4444' : severity === 'high' ? '#f97316' : '#eab308'};">${safeSeverity.toUpperCase()}</td></tr>
        <tr><td style="padding:8px 12px;background:#1e1030;color:#94a3b8;">Title</td>
                      <td style="padding:8px 12px;background:#1a0f2e;">${safeTitle}</td></tr>
      ...
      <div style="background:#1e1030;padding:16px;border-radius:8px;border-left:3px solid #f97316;white-space:pre-wrap;">${safeDescription}</div>
  ...
  const emailResult = await sendEmail({
    to: process.env.SECURITY_INCIDENT_EMAIL || 'support@m20autopilot.com',
