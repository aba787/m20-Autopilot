    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{filtered.length} campaigns</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead style={{ background: 'var(--card-bg)', borderBottom: '1px solid var(--border-primary)' }}>
              <tr>
                <th className="text-left py-2.5 px-3 font-medium" style={{ color: 'var(--text-muted)' }}>Campaign</th>
                <th className="text-left py-2.5 px-3 font-medium" style={{ color: 'var(--text-muted)' }}>Status</th>
                <th className="text-left py-2.5 px-3 font-medium" style={{ color: 'var(--text-muted)' }}>Budget</th>
                <Th label="Spend"  k="spend" />
                <Th label="Sales"  k="sales" />
                <Th label="ROAS"   k="roas"  />
                <Th label="ACOS"   k="acos"  />
                <Th label="CTR"    k="ctr"   />
                <th className="text-left py-2.5 px-3 font-medium" style={{ color: 'var(--text-muted)' }}>Clicks</th>
                <th className="text-left py-2.5 px-3 font-medium" style={{ color: 'var(--text-muted)' }}>Orders</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(c => (
                <tr key={c.id}
                  ...
                  <td className="py-3 px-3" style={{ color: 'var(--text-muted)' }}>${c.budget.toLocaleString()}</td>
                  <td className="py-3 px-3 text-white">${c.spend.toLocaleString()}</td>
                  <td className="py-3 px-3 font-medium" style={{ color: 'var(--success)' }}>${c.sales.toLocaleString()}</td>
                  <td className="py-3 px-3">
