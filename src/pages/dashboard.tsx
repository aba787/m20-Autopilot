...
const kpis = [
  { label: 'المبيعات',  value: kpiData.sales.toLocaleString() + ' ر.س',  icon: TrendingUp,         change: '+28%', up: true  },
  { label: 'الطلبات',   value: kpiData.orders.toLocaleString(),            icon: ShoppingCart,        change: '+12%', up: true  },
  ...
  { label: 'الربح',     value: kpiData.profit.toLocaleString() + ' ر.س',  icon: TrendingUp,          change: '+34%', up: true  },
    ...
    <div className="space-y-5">
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 p-5" style={CARD}>
          <h3 className="font-bold text-sm mb-4 text-white">المبيعات والإنفاق</h3>
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
              ...
              <Line type="monotone" dataKey="sales" stroke="#10b981" strokeWidth={2} dot={false} name="المبيعات" />
              <Line type="monotone" dataKey="spend" stroke={CYAN} strokeWidth={2} dot={false} name="الإنفاق" strokeDasharray="4 2" />
            </LineChart>
          </ResponsiveContainer>
          <div className="flex gap-5 mt-2">
            <span className="flex items-center gap-1.5 text-xs" style={{ color: '#8a94a6' }}>
              <div className="w-4 h-0.5 bg-[#10b981]" /> المبيعات
            </span>
            <span className="flex items-center gap-1.5 text-xs" style={{ color: '#8a94a6' }}>
              <div className="w-4 h-0.5" style={{ background: CYAN }} /> الإنفاق
            </span>
          </div>
        </div>
        ...
        <div className="p-5" style={CARD}>
          <h3 className="font-bold text-sm mb-4 text-white">توزيع الحملات</h3>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <div className="p-5" style={CARD}>
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-bold text-sm text-white flex items-center gap-1.5">
              <AlertTriangle className="w-4 h-4 text-amber-400" /> التنبيهات
            </h3>
            <Link href="/alerts" className="text-xs transition-colors" style={{ color: CYAN }}>عرض الكل →</Link>
          </div>
        ...
        <div className="p-5" style={CARD}>
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-bold text-sm text-white flex items-center gap-1.5">
              <Zap className="w-4 h-4" style={{ color: CYAN }} /> أداء المنتجات
            </h3>
            <Link href="/products" className="text-xs transition-colors" style={{ color: CYAN }}>عرض الكل →</Link>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(0,217,255,0.1)' }}>
                  {['المنتج', 'المبيعات', 'التكلفة', 'الربح', 'الوحدات'].map(h => (
                    <th key={h} className="text-right pb-2 font-medium" style={{ color: '#8a94a6' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
