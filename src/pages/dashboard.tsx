...
export default function Dashboard() {
  ...
  const saveBudget = async () => {
    ...
    try {
      const res = await af('/api/settings', { method: 'PATCH', body: JSON.stringify({ daily_budget: num }) });
      if (!res.ok) { setBudgetError(lang === 'ar' ? 'فشل الحفظ، حاول مرة أخرى' : 'Save failed, try again'); return; }      fetchDash();
    } catch { setBudgetError(lang === 'ar' ? 'فشل الحفظ، حاول مرة أخرى' : 'Save failed, try again'); return; }    setBudgetError('');
    setEditingIdx(null);
  };
