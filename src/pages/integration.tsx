    ...
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-white flex items-center gap-2">
          <Link2 className="w-5 h-5" style={{ color: '#00d9ff' }} /> Amazon Connect
        </h1>
        <p className="text-sm" style={{ color: '#8a94a6' }}>Manage Amazon account connections and data sync</p>
      </div>
            ...
            <div key={store.id} className="p-4" style={CARD}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs" style={{ color: '#4a5568' }}>{store.marketplace}</p>
                </div>
                <div className="flex-shrink-0 text-right">
                  <p className="text-xs" style={{ color: '#4a5568' }}>Last sync</p>
                  <p className="text-xs font-medium text-white">{store.lastSync}</p>
                </div>
              </div>

              <div className="grid grid-cols-3 gap-3 mt-3 pt-3" style={{ borderTop: '1px solid rgba(0,217,255,0.1)' }}>
                <div className="text-center">
                  <p className="text-xs" style={{ color: '#4a5568' }}>Sales</p>
                  <p className="font-bold text-sm" style={{ color: '#10b981' }}>${store.totalSales.toLocaleString()}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs" style={{ color: '#4a5568' }}>Active Campaigns</p>
                  <p className="font-bold text-sm text-white">{store.activeCampaigns}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs" style={{ color: '#4a5568' }}>Account Health</p>
      ...
      <div className="p-4 flex items-center justify-between" style={CARD}>
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: '#4a5568' }}>Data syncs automatically every 6 hours</p>
          </div>
        </div>
        <button onClick={syncNow} disabled={syncing}
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-[#0a0612] transition-all"
          style={{ background: 'linear-gradient(135deg,#00d9ff,#00f0ff)', opacity: syncing ? 0.7 : 1 }}>
        ...
        <div className="space-y-2">
          {['Read advertising data', 'Manage campaigns and keywords', 'Read sales data'].map(p => (
            <div key={p} className="flex items-center gap-2 text-sm">
              ...
              <span style={{ color: '#a0aec0' }}>{p}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ ...CARD, overflow: 'hidden' }}>
        <div className="px-4 py-3 flex items-center gap-1.5" style={{ borderBottom: '1px solid rgba(0,217,255,0.1)' }}>
          <Clock className="w-4 h-4" style={{ color: '#8a94a6' }} />
          <h3 className="font-bold text-sm text-white">Sync History</h3>
        </div>
        <div>
          {syncHistory.map((h, i) => (
            <div key={i} className="flex items-center gap-3 px-4 py-3"
              style={{ borderBottom: i < syncHistory.length - 1 ? '1px solid rgba(0,217,255,0.06)' : 'none' }}>
              ...
              <span className="text-xs flex-shrink-0" style={{ color: '#4a5568' }}>{h.date}</span>
            </div>
          ))}
        </div>
      </div>
