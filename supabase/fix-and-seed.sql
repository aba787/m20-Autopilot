...
-- ============================================================
-- Ensure one Amazon connection per (user_id, profile_id)
-- Idempotent: only adds the UNIQUE constraint if it is missing.
-- (Reconciled from the legacy schema.sql so it actually applies here.)
-- ============================================================
DO $$
BEGIN
IF NOT EXISTS (
  SELECT 1 FROM pg_constraint WHERE conname = 'amazon_connections_user_profile_key'
  ) THEN
ALTER TABLE amazon_connections
ADD CONSTRAINT amazon_connections_user_profile_key UNIQUE (user_id, profile_id);
END IF;
END $$;
